...
export default function CatalogPage() {
  ...
  async function addToCart(productId) {
    ...
    try {
      ...
      setMsg({ type: 'error', text: err.response?.data?.message || 'No se pudo agregar al carrito.' });
    }
  }
  ...
  return (
    <div className="container page">
      ...
      {!loading && !error && (
        <>
          <p className="small" style={{ marginTop: 16 }}>{q.trim() ? `Resultados encontrados: ${filteredProducts.length}` : `Productos disponibles: ${filteredProducts.length}`}</p>
