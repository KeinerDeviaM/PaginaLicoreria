...
export default function AdminWorkersPage() {
  ...
  const filteredWorkers = useMemo(
    () => workers.filter((worker) => JSON.stringify(worker).toLowerCase().includes(search.toLowerCase())),
    [workers, search]
  );

  const filteredUsers = useMemo(
    () => users.filter((u) => JSON.stringify(u).toLowerCase().includes(search.toLowerCase())),
    [users, search]
  );
    ...
    <div className="page">
      <section className="card reveal-on-scroll hero" style={{ marginBottom: 18 }}>
        <div className="stack" style={{ justifyContent: 'space-between', alignItems: 'end', gap: 16, flexWrap: 'wrap' }}>
          <div>
            <div className="small" style={{ color: '#d4af37', marginBottom: 8 }}>Gestión interna</div>
            <h1>Trabajadores y usuarios</h1>
            <p className="small">Crea trabajadores y consulta las cuentas registradas.</p>
          </div>
      ...
      <div className="stack" style={{ marginBottom: 12 }}>
        <button className={`btn ${tab === 'TRABAJADORES' ? 'btn-primary' : 'btn-outline'}`} onClick={() => setTab('TRABAJADORES')}>
          Trabajadores
        </button>
        <button className={`btn ${tab === 'USUARIOS' ? 'btn-primary' : 'btn-outline'}`} onClick={() => setTab('USUARIOS')}>
          Cuentas de usuarios
        </button>
      </div>
        ...
        <section className="card reveal-on-scroll">
          ...
          <form onSubmit={onSubmit}>
            ...
            <div className="stack" style={{ gap: 10 }}>
              ...
              {editing && <button type="button" className="btn btn-outline" onClick={cancelEdit}>Cancelar</button>}
            </div>
          </form>
        </section>
            ...
            <>
              <div className="table-header">
                <div>
                  ...
                  <p className="small">{filteredWorkers.length} resultados visibles.</p>
                </div>
              </div>

              {filteredWorkers.map((worker) => (
                <div key={worker.id} style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 14, marginBottom: 12 }}>
                  <div className="stack" style={{ justifyContent: 'space-between', alignItems: 'start', gap: 12 }}>
                    ...
                    <button className="btn btn-outline" onClick={() => startEdit(worker)}>Editar</button>
                  </div>
                </div>
              ))}
            </>
            ...
            <>
              <div className="table-header">
                <div>
                  ...
                  <p className="small">{filteredUsers.length} resultados visibles.</p>
                </div>
              </div>
