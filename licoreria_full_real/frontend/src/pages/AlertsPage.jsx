  ...
  return (
    <div className="page">
      <section className="card reveal-on-scroll hero" style={{ marginBottom: 18 }}>
        <div>
          <div className="small" style={{ color: '#d4af37', marginBottom: 8 }}>Monitoreo de inventario</div>
      ...
      {!rows.length ? (
        <div className="notice">No hay alertas activas.</div>
      ) : (
        <section className="card reveal-on-scroll" style={{ border: '1px solid rgba(255,193,7,0.28)', background: 'rgba(255,193,7,0.06)' }}>
          <div className="table-header">
            <div>
              <h3>Productos con stock bajo</h3>
              <p className="small">{rows.length} alertas activas.</p>
            </div>
          </div>
                ...
                {rows.map((r) => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td>{minimum}</td>
                      <td><strong style={{ color: '#ffd666' }}>{missing}</strong></td>
                    </tr>
                  );
                })}
              </tbody>
