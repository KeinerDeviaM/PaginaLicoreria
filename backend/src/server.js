...
function auth(req, res, next) {
  ...
  if (!user) return res.status(401).json({ message: 'Usuario inv谩lido' });
  req.user = user;
  req.db = db;
  next();
}
...
simpleCollectionRoutes('categories', 'categories', 'Categor铆a');
...
app.post('/api/orders', auth, requireRoles('CLIENTE'), (req, res) => {
  const db = req.db;
  const cart = db.carts.find(c => c.userId === req.user.id);
  if (!cart || !cart.items.length) return res.status(400).json({ message: 'El carrito est谩 vac铆o' });
  const { deliveryType, deliveryAddress = '', notes = '' } = req.body;
  if (deliveryType === 'DOMICILIO' && !deliveryAddress) {
    return res.status(400).json({ message: 'La direcci贸n es obligatoria para domicilio' });
  }
  const details = [];
  let subtotal = 0;
  for (const item of cart.items) {
    const product = db.products.find(p => p.id === item.productId);
    if (!product || !product.active) return res.status(400).json({ message: `Producto inv谩lido: ${item.productId}` });
  ...
  cart.items = [];
  adminNotify(db, 'PEDIDO_CREADO', 'Nuevo pedido creado', `El cliente ${req.user.firstName} ${req.user.lastName} cre贸 el pedido ${order.number} por valor de ${order.total}.`, req.user, 'PEDIDO', order.id);
  saveDb(db);
...
app.post('/api/payments', auth, requireRoles('CLIENTE'), (req, res) => {
  ...
  adminNotify(db, 'PAGO_REGISTRADO', 'Pago registrado por cliente', `El cliente ${req.user.firstName} ${req.user.lastName} registr贸 un pago para el pedido ${order.number} por valor de ${payment.amount}.`, req.user, 'PAGO', payment.id);
  saveDb(db);
...
function approveReject(req, res, newStatus) {
  ...
  if (newStatus === 'APROBADO') {
    ...
    for (const d of order.details) {
      ...
      db.movements.push({
        ...
        quantity: d.quantity,
        reason: 'Venta',
        notes: `Descuento por aprobaci贸n de pago ${payment.id}`,
        createdAt: now()
      });
    }
    order.status = 'PAGADO';
  ...
  adminNotify(
    ...
    `El ${req.user.role} ${req.user.firstName} ${req.user.lastName} ${newStatus === 'APROBADO' ? 'aprob贸' : 'rechaz贸'} el pago del pedido ${order.number} del cliente ${user.firstName} ${user.lastName}.`,
    req.user,
    'PAGO',
    payment.id
  );
...
app.post('/api/invoices/generate/:orderId', auth, requireRoles('ADMIN', 'TRABAJADOR'), (req, res) => {
  ...
  adminNotify(db, 'FACTURA_GENERADA', 'Factura generada', `El ${req.user.role} ${req.user.firstName} ${req.user.lastName} gener贸 la factura ${invoice.number} del pedido ${order.number} del cliente ${customer.firstName} ${customer.lastName}.`, req.user, 'FACTURA', invoice.id);
  saveDb(db);
...
app.post('/api/users/workers', auth, (req, res) => {
  ...
  if (!String(firstName).trim() || !String(lastName).trim() || !String(email).trim() || !String(password).trim()) {
    return res.status(400).json({
      success: false,
      message: 'Nombre, apellido, correo y contrase帽a son obligatorios'
    });
  }
...
app.put('/api/users/profile/password', auth, (req, res) => {
  ...
  if (!String(currentPassword).trim() || !String(newPassword).trim()) {
    return res.status(400).json({
      success: false,
      message: 'La contrase馻 actual y la nueva contrase馻 son obligatorias'
    });
  }
  ...
  if (String(currentUser.password) !== String(currentPassword)) {
    return res.status(400).json({
      success: false,
      message: 'La contrase馻 actual no es correcta'
    });
  }
  ...
  if (String(newPassword).trim().length < 6) {
    return res.status(400).json({
      success: false,
      message: 'La nueva contrase馻 debe tener al menos 6 caracteres'
    });
  }
  ...
  return res.json({
    success: true,
    message: 'Contrase馻 actualizada correctamente'
  });
});
